...
#' Only one of `peptides.df` or `peptides.list` should be provided.
#' @param peptides.df a data frame containing peptide data (returned by
#' [get_LBCE()] or [consolidate_data()]). Only one of `peptides.df` or `peptides.list`
#' should be provided.
...
#' @param orgID_column name of the column in `peptides.df` or `peptides.list$df`
#' containing the source organism IDs
#' @param hostID_column name of the column in `peptides.df` or `peptides.list$df`
#' containing the host IDs
...
taxonomy_filter <- function(peptides.list = NULL,
                            peptides.df   = NULL,
                            tax_list      = NULL,
                            orgIDs        = NULL,
                            hostIDs       = NULL,
                            removeIDs     = NULL,
                            orgID_column  = ifelse(is.null(peptides.df), "Info_organism_id", "sourceOrg_id"),
                            hostID_column = ifelse(is.null(peptides.df), "Info_host_id", "host_id")) {
  ...
  assertthat::assert_that(
    xor(is.null(peptides.df), is.null(peptides.list)),
    is.null(peptides.df) | is.data.frame(peptides.df),
  ...
  if(is.data.frame(peptides.df)) return(taxonomy_filter_df(peptides.df, tax_list,
