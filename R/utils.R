...
# Check if objecti has class 'peptide.list'
...
# Extract unique entries from comma-separated character strings
...
# Find points where a vector changes values
...
make_windows <- function(x, Class, window_size){
  windows <- sapply(1:length(x),
                          function(pos, x){
                            idx <- (pos - floor(window_size/2)):(pos + floor(window_size/2))
